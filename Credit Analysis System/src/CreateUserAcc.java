...
public class CreateUserAcc extends JFrame {
	...
	public CreateUserAcc() {
	    ...
	    setResizable(false);
	    setContentPane(new JLabel(new ImageIcon("F:\\java\\Projects\\Credit Analysis System\\Images\\CreateUserAcc.jpg")));
	    Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();
	    this.setLocation(dim.width/2-this.getSize().width/2, dim.height/2-this.getSize().height/2);
	
		labelName.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelName.setBounds(49, 107, 96, 22);
		getContentPane().add(labelName);
		
		
		labelgender.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelgender.setBounds(49, 186, 96, 22);
		getContentPane().add(labelgender);
		
		
		labelPhone.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelPhone.setBounds(49, 337, 109, 22);
		getContentPane().add(labelPhone);
		
		
		labelAddress.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelAddress.setBounds(49, 427, 109, 22);
		getContentPane().add(labelAddress);
		
		
		labelDateBirth.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelDateBirth.setBounds(50, 257, 137, 28);
		getContentPane().add(labelDateBirth);
		
	
		labelUserName.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelUserName.setBounds(488, 174, 114, 22);
		getContentPane().add(labelUserName);
		
		
		labelPass.setFont(new Font("Agency FB", Font.BOLD, 22));
		labelPass.setBounds(488, 273, 151, 28);
		getContentPane().add(labelPass);
		

		
		labelUserType.setFont(new Font("Agency FB", Font.BOLD, 22));
		...
		txtPassField.setFont(new Font("Agency FB", Font.PLAIN, 20));
		
	
		txtPassField.setBounds(641, 270, 275, 40);
	    getContentPane().add(txtPassField);
		userTypeComboBox.setFont(new Font("Agency FB", Font.PLAIN, 20));
		...
		btCancel.addActionListener(new ActionListener() {
		 ...
		 btSubmit.addActionListener(new ActionListener() 
		 {
				public void actionPerformed(ActionEvent e) {
					 try
						{				 // insertion in user_info table
						 	String sql1 ="INSERT into user_info (Name,Username,Password,Gender,DOB,User_Type,Phone,Address) values(?,?,?,?,?,?,?,?)";
						    ...
						    String UserType  =userTypeComboBox.getSelectedItem().toString();
							 ...
							 ResultSet rs= null;
							 String sql2 = "Select * from user_login where user_id=(select max(user_id) from user_login)";
							 PreparedStatement st2 = (PreparedStatement) connection.prepareStatement(sql2);
							 ...
							 //insert operation in user_login table
							 String sql3 = "Insert into user_login(user_id,username,password) values(?,?,?)";
							 PreparedStatement st3 = (PreparedStatement) connection.prepareStatement(sql3);
							
							newId = newId+1;
						
							st3.setInt(1, newId);
							st3.setString(2, txtUserName.getText());
							st3.setString(3, txtPassField.getText());
