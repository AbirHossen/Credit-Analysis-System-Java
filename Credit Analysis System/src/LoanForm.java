...
public class LoanForm extends JFrame //implements ActionListener
{
	...
	{	
		...
		{ 
			...
			if(count==1)
				{
					String quer22 = "Select * from loan Where Account_No=? and Loan_Status=?";
					PreparedStatement pst2 = (PreparedStatement) connection.prepareStatement(quer22);
					pst2.setString(1, txtAccountNo.getText());
					pst2.setString(2, "Pending");
					ResultSet rs2 = pst2.executeQuery();
					
					if(rs2.next()) {
						JOptionPane.showMessageDialog(null,"Account already has a pending Loan request");
					}
					else {
						loanSubmit();
					}
	...
	// method for submission of loan
		...
		public void loanSubmit()
		{
			...
			{
				if(Double.parseDouble(txtInterest.getText())<=100) {
					try
					{
					     ...
					     double per = (am*interest)/100;
					     am += per;
					 	 String query ="insert into loan (Account_No,Loan_Amount,Monthly_Pay,Period,Interest,Loan_Status,CDate,Main_Loan) values(?,?,?,?,?,?,?,?)";
			...
			{
				JOptionPane.showMessageDialog(null,"Interest can not be bigger than 100%");
			}
				}
			else {
