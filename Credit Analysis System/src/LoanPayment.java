...
public class LoanPayment extends JFrame {
	...
	public LoanPayment() {
		...
		setResizable(false);
		setContentPane(new JLabel(new ImageIcon("F:\\java\\Projects\\Credit Analysis System\\Images\\LoanPayment.jpg")));
					 ...
					 try
						{			 
							...
							while(rs.next())
								...
								String stat= rs.getString("LOAN_STATUS");
		 ...
		 update.addActionListener(new ActionListener() 
		 {
				public void actionPerformed(ActionEvent e) 
				{
					if(Double.parseDouble(paymentAmountTxt.getText())>0)
					{
				    	...
				    	{
				    		ResultSet rs = null;
					    	String query1 = "Select Loan_Amount from loan where Account_No=?";
							...
							if(rs.next()) {
							    ...
							    double las = rs.getDouble("LOAN_AMOUNT");	
								
						    	if(payAmount > las ) {
						    		JOptionPane.showMessageDialog(null,"Amount is bigger than Loan");
						    	}
						    	else {
						    		//insert into loan payment
							    	...
							    	//loan table update amount
							    	double remain = las - payAmount;
							    	String query3 = "UPDATE Loan set LOAN_AMOUNT="+remain+" where Account_No=? and Loan_Status=?";
							    	...
							    	if(remain == 0 ) {
							    		String query4 = "update loan set Loan_Status=? where Account_No='"+accNumTxt.getText()+"' and Loan_Status=?";
							    		PreparedStatement ps3 = (PreparedStatement) connection.prepareStatement(query4);
							    		ps3.setString(2, "Approved");
								    	ps3.setString(1,"Paid");
								    	ps3.executeUpdate();
							    	}
					...
					{
						JOptionPane.showMessageDialog(null,"Amount Value Can not be zero or negative!");
					}
				}
			}
